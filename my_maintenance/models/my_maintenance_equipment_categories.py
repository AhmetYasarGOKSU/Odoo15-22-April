...
class EquipmentCategories(models.Model):
    ...
    equipments_id = fields.Many2one('equipments')

    def _compute_equipment_count(self):
        for rec in self:
            equipment_count = self.env['equipments'].search_count([('equipment_category_id', '=', rec.id)])
            rec.equipment_count = equipment_count

    def _compute_maintenance_count(self):
        for rec in self:
            maintenance_count = self.env['equipments'].search_count([('equipment_category_id', '=', rec.id)])
